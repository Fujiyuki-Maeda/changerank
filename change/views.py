        ...
        if form.is_valid():
            try:
                ...
                for index, row in df.iloc[1:].iterrows():
                    ...
                    code_10 = int(row[0]); name_10 = str(row[1]); cat_10, _ = Category.objects.get_or_create(code=code_10, level=10, defaults={'name': name_10})
                    if cat_10.name != name_10: cat_10.name = name_10; cat_10.save()
                    code_35 = int(row[2]); name_35 = str(row[3]); cat_35, _ = Category.objects.get_or_create(code=code_35, level=35, defaults={'name': name_35, 'parent': cat_10})
                    if cat_35.name != name_35 or cat_35.parent != cat_10: cat_35.name = name_35; cat_35.parent = cat_10; cat_35.save()
                    code_90 = int(row[4]); name_90 = str(row[5]); cat_90, _ = Category.objects.get_or_create(code=code_90, level=90, defaults={'name': name_90, 'parent': cat_35})
                    if cat_90.name != name_90 or cat_90.parent != cat_35: cat_90.name = name_90; cat_90.parent = cat_35; cat_90.save()
                    code_180 = int(row[6]); name_180 = str(row[7]); cat_180, _ = Category.objects.get_or_create(code=code_180, level=180, defaults={'name': name_180, 'parent': cat_90})
                    if cat_180.name != name_180 or cat_180.parent != cat_90: cat_180.name = name_180; cat_180.parent = cat_90; cat_180.save()
                messages.success(request, "部門マスタの取り込みが完了しました！")
                return redirect('admin:index')
            except Exception as e: messages.error(request, f"エラーが発生しました: {e}")
            ...
            try:
                ...
                for index, row in df.iloc[data_start_row:].iterrows():
                    ...
                    for shop_info in shop_columns:
                        ...
                        SalesRecord.objects.update_or_create(shop=shop, category=category_obj, date=report_date, defaults={'amount_sales': sales_val, 'amount_profit': profit_val})
                    count += 1
...
def student_dashboard(request):
    ...
    for year in years:
        ...
        dept_totals = {}
        for r in records:
            cat_id = r['category_id']; amount = r['amount_sales']; target_id = cat_map.get(cat_id)
            if target_id: dept_totals[target_id] = dept_totals.get(target_id, 0) + amount
    ...
    context = {'years': years, 'table_data': table_data, 'breadcrumbs': breadcrumbs, 'current_title': parent_category.name if parent_category else "全社（10部門）"}
...
def shop_ranking(request):
    ...
    context = {'years': years, 'table_data': table_data, 'checkbox_shops': checkbox_shops, 'selected_shop_ids': selected_shop_ids, 'all_10_depts': all_10_depts, 'selected_dept_code': selected_dept_code, 'selected_dept_name': selected_dept_name}
    return render(request, 'shop_ranking.html', context)

def profit_ranking(request):
    """
    粗利率ランキング（年表形式）
    ★修正: 10年分の推移を計算するように変更
    """
    all_dates = SalesRecord.objects.dates('date', 'day')
    years = sorted(list(set([d.year for d in all_dates])))
    
    if not years:
        return render(request, 'profit_ranking.html', {'error': 'データがまだありません。'})
